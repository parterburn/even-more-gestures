...
- Total: **33 tests, zero failures**.
- Release app successfully compiled for macOS 14 with Xcode 27 / Swift 6.4.
- `codesign --verify --deep --strict` and stapler validation pass on the `0.2.25 (28)` Developer ID-signed bundle with hardened runtime and secure timestamps, including the embedded Sparkle framework.
...
- ChatGPT, Maps, and Chrome defaults are present: `⌘B` / `⌘⌥B`, `⌘⌃S`, and `⌘⇧L` / `⌃G` respectively.
...
- The `0.2.25` build was accepted by Apple’s Notary service and stapled. Its GitHub Release archive is Sparkle-signed and listed in the repository appcast.
